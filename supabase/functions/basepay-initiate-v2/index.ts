...
import * as crypto from 'std/crypto/mod.ts';
const corsHeaders = {
...
serve(async (req) => {
  ...
  try {
    ...
    const { planId, countryCode, localCurrency = 'NGN' } = await req.json();
    ...
    const localAmount = (plan.deposit_usd / 100) * fxRate;
    ...
    const siteUrl = req.headers.get("origin") || "https://yourapp.com";
